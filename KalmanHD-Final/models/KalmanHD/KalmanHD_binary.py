...
except ModuleNotFoundError:  # torchhd >= 5.x renomeou torchhd.map.MAP -> torchhd.MAPTensor
    from torchhd import MAPTensor as MAP
            ...
            for n in tqdm(range(size)): # For each ts
                ...
                for i in (sets_training): # For each set in the rolling window
                    ...
                    """if (i % 5000 == 0):
                        ...
                        print(f"\nCross Validation root mean squared error of {(mean_squared_error(labels_full, pred) ** 0.5):.3f}")"""
